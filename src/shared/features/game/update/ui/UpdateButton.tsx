...
type UpdateButtonProps = Omit<ComponentProps<typeof ShimmerButton>, 'isBusy'> & {
    isBusy?: boolean
    percent?: number | null
}
...
const BusyContent = ({ percent }: { percent?: number | null }) => (
    <span className="flex items-center gap-3">
        ...
        <GradientText className='text-[14px] font-extrabold uppercase tracking-[0.22em] bg-linear-to-b from-foreground to-shimmer-obsidian via-none'>Обновление</GradientText>
        {percent != null && (
...
const UpdateButton = ({ className, children, isBusy, percent, size, variant, ...rest }: UpdateButtonProps) => (
    <ShimmerButton
        variant={variant}
        isBusy={isBusy}
        size={size}
    ...
    >
        ...
        >
            <HiArrowPath className="w-4 h-4 shrink-0" />
        </motion.span>
    </ShimmerButton>
)
