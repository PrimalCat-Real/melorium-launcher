...
const DownloadGroup = () => {
    ...
    useEffect(() => {
        const unlisten = listen<{ fraction: number | null; message: string; event: { type: string } }>(
            'loading',
            (event) => {
                const payload = event.payload as { fraction: number | null; message: string; event?: { type: string } }
                const fraction = payload.fraction
                const barType = payload.event?.type ?? 'unknown'

                const current = useGameStateStore.getState().download
                const prevCompleted = current.status === 'downloading' ? current.completedBarTypes : []
                const prevFileEvents = current.status === 'downloading' ? current.fileEvents : 0
                const prevDownloadedBytes = current.status === 'downloading' ? current.downloadedBytes : null
                const prevTotalBytes = current.status === 'downloading' ? current.totalBytes : null
                ...
                if (history.length >= 3) {
                    ...
                    if (timeDiff > 0 && fracDiff > 0) {
                        const etaSecs = Math.round((1 - newest.fraction) / (fracDiff / timeDiff))
                ...
                const fileEvents = barType === 'pack_file_download' ? prevFileEvents + 1 : prevFileEvents
        ...
        const unlisten = listen<{ downloaded: number; total: number }>(
            ...
            (event) => {
                ...
                if (prev && downloaded > prev.bytes && now - prev.time >= 200) {
                    const mbPerSec = (downloaded - prev.bytes) / 1024 / 1024 / ((now - prev.time) / 1000)
                ...
                if (current.status === 'downloading') {
                    setDownload({ ...current, downloadedBytes: downloaded, totalBytes: total, ...(speed ? { speed } : {}) })
                }
            },
        )
        return () => { unlisten.then((fn) => fn()) }
    ...
    const handleInstall = async () => {
        ...
        setDownload({ status: 'downloading', percent: 0, speed: '', eta: '', stage: 'Начало установки...', barType: '', completedBarTypes: [], fileEvents: 0, downloadedBytes: null, totalBytes: null })
        try {
            const filesPresent = await isModpackPresent(rawPath)
            await invoke('install_or_update_modpack', { path: rawPath, force: !filesPresent })
            setDownload({ status: 'ready' })
        } catch (err) {
        ...
        <div className="flex flex-col gap-2">
            {isReady ? (
                ...
                <DownloadButton
                    ...
                    isBusy={isBusy}
                    percent={percent}
                    className={cn('font-bold py-6 rounded-lg shadow-2xl', isBusy && 'pointer-events-none')}
