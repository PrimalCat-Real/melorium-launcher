...
/*
   ...
   For example:
   pub async fn loading_function() -> crate::modrinth::Result<()> {
...
pub async fn init_loading_unsafe(
    ...
    let event_state = crate::modrinth::EventState::get()?;
...
pub async fn emit_warning(message: &str) -> crate::modrinth::Result<()> {
    {
        let event_state = crate::modrinth::EventState::get()?;
        event_state
            .app
...
pub async fn emit_command(command: CommandPayload) -> crate::modrinth::Result<()> {
    tracing::debug!("Command: {}", serde_json::to_string(&command)?);
    {
        ...
        if let Some(window) = event_state.app.get_webview_window("main") {
...
#[allow(unused_variables)]
pub async fn emit_friend(payload: FriendPayload) -> crate::modrinth::Result<()> {
    {
        let event_state = crate::modrinth::EventState::get()?;
        event_state
            .app
...
macro_rules! count {
    () => (0usize);
    ( $x:tt $($xs:tt)* ) => (1usize + $crate::modrinth::count!($($xs)*));
}
#[macro_export]
    ...
    ($key:expr, $total:expr, $message:expr; $($task:expr $(,)?)+) => {
        {
            ...
            let num_futures = $crate::modrinth::count!($($task)*);
                ...
                $( let [ <unique_name $task>] = {
                    {
                        ...
                        async move {
                            let res = $task.await;
                            if let Some(key) = key {
                                $crate::modrinth::event::emit::emit_loading(key, increment, message)?;
                            }
                            res
                        }
                    }
...
where
    I: futures::TryStreamExt<Error = crate::modrinth::Error> + TryStream<Ok = T>,
    F: FnMut(T) -> Fut + Send,
    Fut: Future<Output = crate::modrinth::Result<()>> + Send,
    T: Send,
{
    let mut f = f;
    stream
