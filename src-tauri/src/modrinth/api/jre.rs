...
use crate::modrinth::state::JavaVersion;
use crate::modrinth::util::fetch::{fetch_advanced, fetch_json};
use dashmap::DashMap;
use reqwest::Method;
...
use crate::modrinth::util::jre::extract_java_version;
use crate::modrinth::{
    LoadingBarType, State,
    util::jre::{self},
};

pub async fn get_java_versions() -> crate::modrinth::Result<DashMap<u32, JavaVersion>> {
    let state = State::get().await?;

    JavaVersion::get_all(&state.pool).await
}

pub async fn set_java_version(java_version: JavaVersion) -> crate::modrinth::Result<()> {
    let state = State::get().await?;
...
) -> crate::modrinth::Result<Vec<JavaVersion>> {
...
pub async fn auto_install_java(java_version: u32) -> crate::modrinth::Result<PathBuf> {
    let state = State::get().await?;
    ...
    if let Some(download) = packages.first() {
        let file_result = fetch_advanced(
            Method::GET,
...
pub async fn check_jre(path: PathBuf) -> crate::modrinth::Result<JavaVersion> {
