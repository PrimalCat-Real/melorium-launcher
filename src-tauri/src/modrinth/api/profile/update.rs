use crate::modrinth::state::CacheBehaviour;
use crate::modrinth::{
    LoadingBarType,
    event::{
        ProfilePayloadType,
...
pub async fn repair_managed_modrinth(profile_path: &str) -> crate::modrinth::Result<()> {
    ...
    for (file, _) in projects_map {
        crate::modrinth::state::Profile::remove_project(&profile.path, &file).await?;
    }
...
async fn replace_managed_modrinth(
    profile_path: &str,
    profile: &crate::modrinth::state::Profile,
    project_id: &String,
    version_id: &String,
    new_version_id: Option<&String>,
    ignore_lock: bool,
) -> crate::modrinth::Result<()> {
    crate::modrinth::profile::edit(profile_path, |profile| {
