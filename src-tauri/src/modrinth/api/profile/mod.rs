...
use crate::modrinth::util::fetch;
use crate::modrinth::util::io::{self, IOError};
pub use crate::modrinth::{State, state::Profile};
...
use crate::modrinth::data::Settings;
use crate::modrinth::server_address::ServerAddress;
use dashmap::DashMap;
...
/// Remove a profile
#[tracing::instrument]
pub async fn remove(path: &str) -> crate::modrinth::Result<()> {
...
#[tracing::instrument]
pub async fn get(path: &str) -> crate::modrinth::Result<Option<Profile>> {
    let state = State::get().await?;
...
#[tracing::instrument]
pub async fn get_many(paths: &[&str]) -> crate::modrinth::Result<Vec<Profile>> {
    let state = State::get().await?;
...
) -> crate::modrinth::Result<DashMap<String, ProfileFile>> {
    let state = State::get().await?;
...
#[tracing::instrument]
pub async fn get_full_path(path: &str) -> crate::modrinth::Result<PathBuf> {
    let state = State::get().await?;
...
where
    Fut: Future<Output = crate::modrinth::Result<()>>,
{
    let state = State::get().await?;
...
pub async fn get_optimal_jre_key(
    ...
    if let Some(profile) = get(path).await? {
        ...
        let loader_version = crate::modrinth::launcher::get_loader_version_from_profile(
            &profile.game_version,
            profile.loader,
        ...
        let version_info = crate::modrinth::launcher::download::download_version_info(
            &state,
            version,
...
#[tracing::instrument]
pub async fn list() -> crate::modrinth::Result<Vec<Profile>> {
    let state = State::get().await?;
...
pub async fn update_all_projects(
    profile_path: &str,
) -> crate::modrinth::Result<HashMap<String, String>> {
    if let Some(profile) = get(profile_path).await? {
        ...
        loading_try_for_each_concurrent(
            futures::stream::iter(keys).map(Ok::<String, crate::modrinth::Error>),
            None,
            Some(&loading_bar),
            100.0,
            len,
...
pub async fn get_pack_export_candidates(
    profile_path: &str,
) -> crate::modrinth::Result<Vec<SafeRelativeUtf8UnixPathBuf>> {
    let mut path_list = Vec::new();
...
fn pack_get_relative_path(
    ...
    path: &PathBuf,
) -> crate::modrinth::Result<SafeRelativeUtf8UnixPathBuf> {
...
pub async fn run(
    ...
    let default_account = Credentials::get_default_credential(&state.pool)
        .await?
        .ok_or_else(|| crate::modrinth::ErrorKind::NoCredentialsError.as_error())?;
...
async fn run_credentials(
    ...
    crate::modrinth::launcher::launch_minecraft(
        &java_args,
        &env_args,
        &mc_set_options,
        &wrapper,
...
pub async fn kill(path: &str) -> crate::modrinth::Result<()> {
    let state = State::get().await?;
    let processes = crate::modrinth::api::process::get_by_profile_path(path).await?;
...
#[tracing::instrument]
pub async fn try_update_playtime(path: &str) -> crate::modrinth::Result<()> {
    let state = State::get().await?;
...
pub async fn create_mrpack_json(
    ...
    version_id: String,
    description: Option<String>,
) -> crate::modrinth::Result<PackFormat> {
    // Add loader version to dependencies
    let mut dependencies = HashMap::new();
    match (profile.loader, profile.loader_version.clone()) {
        (crate::modrinth::prelude::ModLoader::Forge, Some(v)) => {
            dependencies.insert(PackDependency::Forge, v)
        }
        (crate::modrinth::prelude::ModLoader::NeoForge, Some(v)) => {
            dependencies.insert(PackDependency::NeoForge, v)
        }
        (crate::modrinth::prelude::ModLoader::Fabric, Some(v)) => {
            dependencies.insert(PackDependency::FabricLoader, v)
        }
        (crate::modrinth::prelude::ModLoader::Quilt, Some(v)) => {
            dependencies.insert(PackDependency::QuiltLoader, v)
        }
        (crate::modrinth::prelude::ModLoader::Vanilla, _) => None,
        _ => {
            return Err(crate::modrinth::ErrorKind::OtherError(
    ...
    let files = projects
        ...
        .filter_map(|(path, version_id)| {
            if let Some(version) = versions.iter().find(|x| x.id == version_id)
            {
                ...
                let Some(primary_file) = version.files.first() else {
                    return Some(Err(crate::modrinth::ErrorKind::OtherError(format!(
                ...
                Some(Ok(PackFile {
                    path: match path.try_into() {
                        Ok(path) => path,
                        Err(_) => {
                            return Some(Err(crate::modrinth::ErrorKind::OtherError(
        ...
        .collect::<crate::modrinth::Result<Vec<PackFile>>>()?;
