...
use crate::modrinth::launcher::get_loader_version_from_profile;
use crate::modrinth::settings::Hooks;
use crate::modrinth::state::{LauncherFeatureVersion, LinkedData, ProfileInstallStage};
use crate::modrinth::util::io::{self, canonicalize};
use crate::modrinth::{ErrorKind, pack, profile};
...
pub async fn profile_create(
    ...
    let result = async {
        ...
        crate::modrinth::state::fs_watcher::watch_profile(
            &profile.path,
            &state.file_watcher,
            &state.directories,
        )
        ...
        if !skip_install_profile.unwrap_or(false) {
            crate::modrinth::launcher::install_minecraft(&profile, None, false).await?;
        }
