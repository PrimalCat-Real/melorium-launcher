use crate::modrinth::ErrorKind;
use crate::modrinth::data::ModrinthCredentials;
use crate::modrinth::event::FriendPayload;
use crate::modrinth::event::emit::emit_friend;
use crate::modrinth::state::tunnel::InternalTunnelSocket;
...
impl FriendsSocket {
    ...
    pub async fn connect(
        ...
        if let Some(credentials) = credentials {
            ...
            request.headers_mut().insert(
                "User-Agent",
                HeaderValue::from_str(&crate::modrinth::launcher_user_agent()).unwrap(),
            );
    ...
    #[tracing::instrument(skip_all)]
    pub async fn socket_loop() -> crate::modrinth::Result<()> {
    ...
    pub async fn disconnect(&self) -> crate::modrinth::Result<()> {
        let mut write_lock = self.write.write().await;
        if let Some(_) = write_lock.take() {
            // Drop connection
        }
        Ok(())
    }
    ...
    pub async fn friends(
        exec: impl sqlx::Executor<'_, Database = sqlx::Sqlite> + Copy,
        semaphore: &FetchSemaphore,
    ) -> crate::modrinth::Result<Vec<UserFriend>> {
        fetch_json(
            Method::GET,
    ...
    ) -> crate::modrinth::Result<TunnelSocket> {
        Ok(TunnelSocket {
            socket_id,
