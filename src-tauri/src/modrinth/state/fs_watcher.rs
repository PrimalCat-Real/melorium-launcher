...
use crate::modrinth::event::ProfilePayloadType;
use crate::modrinth::event::emit::{emit_profile, emit_warning};
use crate::modrinth::state::{
    DirectoryInfo, ProfileInstallStage, ProjectType, attached_world_data,
};
use crate::modrinth::worlds::WorldType;
...
pub async fn init_watcher() -> crate::modrinth::Result<FileWatcher> {
                    ...
                    events.iter().for_each(|e| {
                        ...
                        for component in e.path.components() {
                            ...
                            if component.as_os_str()
                                == crate::modrinth::state::dirs::PROFILES_FOLDER_NAME
                            {
                                found = true;
                            }
                        }
...
fn crash_task(path: String) {
    tokio::task::spawn(async move {
        let res = async {
            let profile = crate::modrinth::api::profile::get(&path).await?;
