...
use crate::modrinth::LoadingBarType;
use crate::modrinth::event::emit::{emit_loading, init_loading};
use crate::modrinth::state::LAUNCHER_STATE;
use crate::modrinth::state::{JavaVersion, Profile, Settings};
use crate::modrinth::util::fetch::IoSemaphore;
use dashmap::DashSet;
...
impl DirectoryInfo {
        ...
        E: sqlx::Executor<'a, Database = sqlx::Sqlite> + Copy,
    {
        ...
        if let Some(ref prev_custom_dir) = settings.prev_custom_dir {
            ...
            fn get_disk_usage(path: &Path) -> crate::modrinth::Result<Option<u64>> {
                let path = crate::modrinth::util::io::canonicalize(path)?;
            ...
            if prev_dir != move_dir {
                ...
                async fn add_paths(
                    ...
                    if !source.exists() {
                        crate::modrinth::util::io::create_dir_all(source).await?;
                    }

                    if !destination.exists() {
                        crate::modrinth::util::io::create_dir_all(destination).await?;
                    }
