...
pub async fn login_begin(
...
) -> crate::modrinth::Result<MinecraftLoginFlow> {
    ...
    match sisu_authenticate(&pair.token.token, &challenge, &pair.key, current_date).await {
        ...
        Err(err) => return Err(crate::modrinth::ErrorKind::from(err).into()),
    }
}
...
impl Credentials {
    ...
    pub async fn get_default_credential(
        exec: impl sqlx::Executor<'_, Database = sqlx::Sqlite> + Copy,
    ) -> crate::modrinth::Result<Option<Credentials>> {
    ...
    pub async fn get_all(
        exec: impl sqlx::Executor<'_, Database = sqlx::Sqlite> + Copy,
    ) -> crate::modrinth::Result<DashMap<Uuid, Self>> {
        let res = sqlx::query!(
            "
            SELECT
                uuid, active, username, access_token, refresh_token, expires
...
impl DeviceTokenPair {
    ...
    async fn refresh_and_get_device_token(
        ...
        exec: impl sqlx::Executor<'_, Database = sqlx::Sqlite> + Copy,
    ) -> crate::modrinth::Result<(Self, DateTime<Utc>)> {
...
impl MinecraftProfile {
    ...
    pub fn current_skin(&self) -> crate::modrinth::Result<&MinecraftSkin> {
        Ok(self
            .skins
            .iter()
