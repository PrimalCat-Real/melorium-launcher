use crate::modrinth::event::emit::{emit_process, emit_profile};
use crate::modrinth::event::{ProcessPayloadType, ProfilePayloadType};
use crate::modrinth::profile;
use crate::modrinth::util::io::IOError;
use crate::modrinth::util::rpc::RpcServer;
...
impl ProcessManager {
    ...
    pub async fn wait_for(&self, id: Uuid) -> crate::modrinth::Result<()> {
        if let Some(mut process) = self.processes.get_mut(&id) {
    ...
    pub async fn kill(&self, id: Uuid) -> crate::modrinth::Result<()> {
        if let Some(mut process) = self.processes.get_mut(&id) {
...
impl Process {
        ...
        let timestamp = timestamp
            ...
            .and_then(|x| {
                Utc.timestamp_opt(x, 0).single().ok_or_else(|| {
                    crate::modrinth::ErrorKind::OtherError(
    ...
    async fn parse_and_insert_server_join(
        ...
        crate::modrinth::state::server_join_log::JoinLogEntry {
    ...
    async fn sequential_process_manager(
        ...
        #[cfg(feature = "tauri")]
        {
            let window = crate::modrinth::EventState::get_main_window().await?;
