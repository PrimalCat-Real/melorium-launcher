...
#[tracing::instrument]
pub async fn check_java_at_filepath(path: &Path) -> crate::modrinth::Result<JavaVersion> {
    ...
    for line in stdout.lines() {
        ...
        if key == "os.arch" {
            java_arch = Some(value.to_string());
    ...
    {
        if let Ok(version_parsed) = extract_java_version(&version) {
            let path = java.to_string_lossy().to_string();
            return Ok(JavaVersion {
                parsed_version: version_parsed,
                path,
                version,
                architecture: arch,
            });
        ...
        return Err(JREError::InvalidJREVersion(version).into());
    }
